...
function Select:initPlayerBtnInfo()
    ...
    self.mView["dec"]:loadTexture("select/job/dec_"..btnTbl.sex..".png")
    --按钮点击事件
    local btnList = {
        [1] = {node = "btnSex1",  idx = 0,  tag = "sex", select_img = "select/res/btnSexIcon_1.png", normal_img = "select/res/btnMan.png"},
        [2] = {node = "btnSex2", idx = 1, tag = "sex", select_img = "select/res/btnSexIcon2.png", normal_img = "select/res/btnWoman.png"},
    }
...
function Select:getLocalSaveInfo()
    ...
    if global.isGrabScreenMode == 1 then 
        local randName = self:getRandPlayerName()
        tbl = {job = 1, sex = math.random(0,1), name = randName}
        global.saveTools.saveData(global.viewData.PLAYER_INFO_SAVE_TO_LOCAL, tbl) 
    end
